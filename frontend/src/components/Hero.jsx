import React, { useState } from 'react';
import LeadSimulationModal from './LeadSimulationModal';
import './Hero.css';

const Hero = () => {
  const [isSimulationOpen, setIsSimulationOpen] = useState(false);

  return (
    <>
      <section className="hero">
        <div className="hero-bg-image"></div>
        <div className="hero-overlay"></div>
        
        <div className="container hero-content">
          <div className="hero-text-box">
            <span className="hero-badge">Energia Inteligente</span>
            <h1 className="hero-title highlight">
              Transforme a luz do sol em economia real
            </h1>
            <p className="hero-description">
              Gere sua própria energia limpa com a DRM. Reduza sua conta de luz em até 82% e valorize seu imóvel com nossos sistemas solares de alta performance.
            </p>
            <div className="hero-actions">
              <button type="button" className="btn btn-primary btn-lg" onClick={() => setIsSimulationOpen(true)}>
                Simule um orçamento agora
              </button>
              <a href="#projetos" className="btn btn-outline btn-lg btn-white">Ver Projetos</a>
            </div>
          </div>
        </div>
        
        <div className="hero-wave">
          <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
            <path d="M321.39,56.44c58-10.79,114.16-30.13,172-41.86,82.39-16.72,168.19-17.73,250.45-.39C823.78,31,906.67,72,985.66,92.83c70.05,18.48,146.53,26.09,214.34,3V120H0V95.8C59.71,118.08,130.83,123.63,195,109.51c59.4-13.1,117.8-38.38,176.39-53.07Z" className="shape-fill"></path>
          </svg>
        </div>
      </section>
      <LeadSimulationModal isOpen={isSimulationOpen} onClose={() => setIsSimulationOpen(false)} />
    </>
  );
};

export default Hero;

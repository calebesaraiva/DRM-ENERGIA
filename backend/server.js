...
const sendWhatsAppTextMessage = async (to, text, options = {}) => {
  ...
  if (activeSocket) {
    ...
    if (options.prepareSession !== false) {
      const sessionTimeoutMs = Number(options.prepareSessionTimeoutMs || 8000);
      try {
        await withTimeout(activeSocket.getUSyncDevices?.([jid], false, false) || Promise.resolve(), sessionTimeoutMs, 'getUSyncDevices');
        await withTimeout(activeSocket.assertSessions?.([jid], true) || Promise.resolve(), sessionTimeoutMs, 'assertSessions');
      } catch (error) {
...
const buildOrcamentoPdf = async (orcamento) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const tableRows = [
      ...
      ...(caboCc ? [['Cabos CC', `${caboCc} m de cabo solar CC`, caboCc, false]] : []),
...
app.post('/api/admin/orcamentos', authRequired, requirePermission('orcamentos'), async (req, res) => {
  ...
  const normalizedDimensionamento = {
    ...
    quantidade_baterias: dimensionamento.bateria_modelo || dimensionamento.bateria_id
      ? Number(dimensionamento.quantidade_baterias || dimensionamento.quantidadeBaterias || 1)
      : '',
